        ...
        <table class="table">
            <tbody>
                ...
                <tr v-for="(item,index) in sp">
                    ...
                    <td :class="$parent.getActiveClass(item.host_corner,sp[index+1>=sp.length?sp.length-1:index+1].host_corner)||$parent.getActiveClass(item.guest_corner,sp[index+1>=sp.length?sp.length-1:index+1].guest_corner)">{{item.host_corner}}:{{item.guest_corner}}</td>
                    <td :class="$parent.getTrendClass(item,sp[index+1>=sp.length?sp.length-1:index+1])">{{item.up_sp||'-'}}</td>
